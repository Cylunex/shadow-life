...
- 固定原件仍保存在资产版本中。图片和最多 8 页 PDF 通过配置的视觉理解端点生成结构化结果：标题、资料日期、分类、摘要、正文、页码与引文位置。PDF 使用 `pdfinfo` / `pdftoppm` 转成页面图像；没有本地 OCR 依赖。
- 初始资料尚未被用户修改时，成功处理会自动建立普通的第 2 版资料修订，并生成对应检索片段；无需人工确认。修订保留处理任务 ID，可追溯模型与处理器版本、源页及引文位置。用户在处理期间已有修改时不覆盖该修订。Web 与 Android 仍可按普通资料修订流程更正。处理任务的认领、失败、重试和完成由 Executor 回执与租约约束。视觉结果不自动创建账目或健康事实。
...
- 临时 PostgreSQL 中的新领域测试覆盖月度核对、清单权限与修订、视觉协议、自动资料修订、用户修订保护、未配置失败和重试。整合分支上完整数据库测试已在独立 PostgreSQL 实例通过（52/52）；未连接真实视觉模型。
