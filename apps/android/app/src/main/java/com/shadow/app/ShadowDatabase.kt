...
@Dao interface CommandDao{
  ...
  @Query("delete from pending_commands where accountId=:account and subjectId=:subject and state in ('committed','blocked','failed')")suspend fun clearTerminalCommands(account:String,subject:String):Int
...
@Database(entities=[PendingCommand::class,PendingAttachment::class],version=5,exportSchema=true)abstract class ShadowDatabase:RoomDatabase(){abstract fun commands():CommandDao}
