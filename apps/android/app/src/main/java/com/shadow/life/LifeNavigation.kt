...
@Composable fun LifeApp(session:ProductSession?,viewModel:NativeLifeViewModel,appearance:Appearance,statusMessage:String?,pendingShare:SharePayload?,notificationAuthorization:String?,openInboxNonce:Long,scaleSettings:ScaleProfileSettings,onAcceptShare:(SharePayload)->Unit,onDiscardShare:()->Unit,onDismissStatus:()->Unit,onAppearance:(Appearance)->Unit,onLogin:()->Unit,onLogout:()->Unit,onHealthSync:()->Unit,onSamsungSync:()->Unit,onScale:()->Unit,onSaveScale:(ScaleProfileSettings)->Unit,onNotificationPermission:()->Unit){
  ...
  Scaffold(containerColor=MaterialTheme.colorScheme.background,bottomBar={if(root)LifeDock(
    ...
    NavHost(navController=nav,startDestination=TodayRoute){
      composable<TodayRoute>{TodayVisualScreen(session.accountId,viewModel.today,viewModel.homeHealth,viewModel.deviceSyncStatus,viewModel.queueStatus,SamsungHealthBridge.available(),{viewModel.refreshToday();viewModel.refreshHomeHealth()},{domain,tab->if(domain==LifeDomain.Library)nav.navigate(LibraryRoute) else{viewModel.loadWorkspace(domain);nav.navigate(WorkspaceRoute(domain.name,tab))}},{nav.navigate(ItemsRoute)},{nav.navigate(PlansRoute)},{nav.navigate(LibraryRoute)},{domain,id,title->viewModel.loadDetail(domain,id);nav.navigate(DetailRoute(domain.name,id,title))},{nav.navigate(RecordsRoute)},{viewModel.refreshInbox();nav.navigate(InboxRoute)},{nav.navigate(FeaturesRoute)},{kind->openComposer(defaultCaptureSeed(kind))},onHealthSync,onSamsungSync,onScale,{nav.navigate(ConnectionsRoute)},{nav.navigate(SettingsRoute)})}
      composable<RecordsRoute>{LaunchedEffect(Unit){viewModel.refreshTimeline();viewModel.loadDay()};RecordsVisualScreen(viewModel.timeline,viewModel.searchResults,viewModel::refreshTimeline,viewModel::loadMoreTimeline,viewModel::search,viewModel::loadMoreSearch,viewModel::clearSearch,{domain,id,title->viewModel.loadDetail(domain,id);nav.navigate(DetailRoute(domain.name,id,title))},{openComposer()},{nav.navigate(ConnectionsRoute)},{nav.navigate(SettingsRoute)},viewModel.selectedDay,viewModel.day,viewModel.memories,viewModel::loadDay,viewModel::loadMoreDay,{entry->when(entry.domain){"items"->nav.navigate(OwnedItemDetailRoute(entry.recordId?:entry.id));"plans"->nav.navigate(PlanDetailRoute(entry.recordId?:entry.id));else->{val domain=LifeDomain.entries.firstOrNull{it.name.equals(entry.domain,true)}?:LifeDomain.Library;viewModel.loadDetail(domain,entry.recordId?:entry.id);nav.navigate(DetailRoute(domain.name,entry.recordId?:entry.id,entry.title))}}})}
      composable<PlansRoute>{LaunchedEffect(Unit){viewModel.refreshPlans()};PlansVisualScreen(
      ...
      composable<LibraryRoute>{LaunchedEffect(Unit){viewModel.refreshLibrary()};LibraryVisualScreen(viewModel.library,viewModel::refreshLibrary,viewModel::loadMoreLibrary,{domain,id,title->viewModel.loadDetail(domain,id);nav.navigate(DetailRoute(domain.name,id,title))},{kind->openComposer(defaultCaptureSeed(kind))},{nav.navigate(ConnectionsRoute)},{nav.navigate(SettingsRoute)})}
      composable<WorkspaceRoute>{backStack->
        ...
        when(domain){
          ...
          LifeDomain.Money->MoneyWorkspaceScreen(
            ...
            viewModel.moneyImportMonth,viewModel.moneyImportReview,viewModel.moneyImportSubmit,viewModel::loadMoneyImportMonth,viewModel::openMoneyImportBatch,viewModel::resolveMoneyImport
          )
          LifeDomain.Travel->TravelWorkspaceScreen(
            viewModel.workspaceOverview,viewModel.workspace,{viewModel.loadWorkspace(domain)},viewModel::loadMoreWorkspace,{nav.popBackStack()},detail,capture,route.tab,viewModel::selectTravelTrip,{seed->openComposer(seed)},viewModel.submit,viewModel::saveTravelDay,viewModel::editAgain,viewModel::saveTravelChecklist,viewModel.offlineItinerary,viewModel::prepareOfflineItinerary
          )
      ...
      composable<DetailRoute>{backStack->val route=backStack.toRoute<DetailRoute>();val domain=LifeDomain.valueOf(route.domain);LaunchedEffect(route.id){viewModel.loadDetail(domain,route.id)};key(route.domain,route.id){DetailScreen(route.title,viewModel.detail,viewModel.submit,{viewModel.loadDetail(domain,route.id)},{nav.popBackStack()},{link->openPlanningRelated(nav,viewModel,link.kind,link.id)},{action->openComposer(action.seed)},viewModel::correct,viewModel::editAgain,{assetId->viewModel.queueLibraryVision(route.id,assetId)},viewModel::retryLibraryVision)}}
      composable<PlanDetailRoute>{backStack->val route=backStack.toRoute<PlanDetailRoute>();LaunchedEffect(route.id){viewModel.loadPlanDetail(route.id)};PlanDetailScreen(viewModel.planDetail,{viewModel.loadPlanDetail(route.id)},{kind,id->openPlanningRelated(nav,viewModel,kind,id)},{nav.popBackStack()},viewModel::saveProjectAction,viewModel.projectActionSubmit)}
