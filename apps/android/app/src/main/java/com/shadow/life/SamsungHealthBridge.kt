...
object SamsungHealthBridge {
  ...
  fun startIfAuthorized(context:Context,accountId:String):Result<Unit> = runCatching{
    check(available()){ "当前安装包未包含 Samsung Health Data SDK" }
    Class.forName("com.shadow.life.SamsungSync").getMethod("startIfAuthorized",Context::class.java,String::class.java).invoke(null,context,accountId)
  }
}
