...
class NativeLifeViewModel(application:Application):AndroidViewModel(application){
  ...
  fun loadMoreInbox(){val current=(inbox as? LoadState.Ready)?.value?:return;val cursor=current.nextCursor?:return;viewModelScope.launch{try{val next=repository.notifications(cursor);inbox=LoadState.Ready(current.copy(items=current.items+next.items,nextCursor=next.nextCursor))}catch(error:CancellationException){throw error}catch(_:Exception){inbox=LoadState.Ready(current)}}}
  ...
  fun loadMoreSearch(){val current=(searchResults as? LoadState.Ready)?.value?:return;val cursor=current.nextCursor?:return;viewModelScope.launch{when(val next=load("没有更多搜索结果"){repository.search(activeSearchQuery,cursor)}){is LoadState.Ready->searchResults=LoadState.Ready(current.copy(items=current.items+next.value.items,nextCursor=next.value.nextCursor,asOf=current.asOf));is LoadState.Failed->searchResults=LoadState.Ready(current);else->Unit}}}
  fun clearSearch(){activeSearchQuery="";searchResults=null}
  fun openAssistant(){if(assistantHistory is LoadState.Loading||assistantHistory is LoadState.Ready)return;assistantHistory=LoadState.Loading;viewModelScope.launch{try{val threadId=assistantThreadId?:repository.recentThreads().firstOrNull()?.id;assistantThreadId=threadId;assistantHistory=if(threadId==null)LoadState.Empty("开始一段新对话") else LoadState.Ready(repository.assistantMessages(threadId))}catch(error:CancellationException){throw error}catch(error:Exception){assistantHistory=LoadState.Failed(error.message?:"无法读取对话")}}}
  fun loadOlderAssistantMessages(){val current=(assistantHistory as? LoadState.Ready)?.value?:return;val cursor=current.nextCursor?:return;viewModelScope.launch{try{val older=repository.assistantMessages(current.threadId,cursor);assistantHistory=LoadState.Ready(current.copy(items=older.items+current.items,nextCursor=older.nextCursor))}catch(error:CancellationException){throw error}catch(_:Exception){assistantHistory=LoadState.Ready(current)}}}
  fun askLife(message:String){if(message.isBlank())return;assistant=LoadState.Loading;viewModelScope.launch{assistant=try{val reply=repository.assist(message,assistantThreadId);assistantThreadId=reply.threadId;assistantHistory=LoadState.Ready(repository.assistantMessages(reply.threadId));LoadState.Ready(reply)}catch(error:CancellationException){throw error}catch(error:Exception){LoadState.Failed(error.message?:"Life 请求失败")}}}
