...
class NativeLifeViewModel(application:Application):AndroidViewModel(application){
  ...
  fun refreshLibrary(query:String=""){val token=beginRead("library");library=LoadState.Loading;viewModelScope.launch{val result=loadList(if(query.isBlank())"资料库还是空的" else "没有符合条件的资料"){repository.library(query)};if(isCurrentRead("library",token))library=result}}
  ...
  fun loadDetail(domain:LifeDomain,id:String){val token=beginRead("detail");detail=LoadState.Loading;viewModelScope.launch{val result=load("对象不可用"){repository.detail(domain,id)};if(isCurrentRead("detail",token))detail=result}}
