...
class NativeLifeViewModel(application:Application):AndroidViewModel(application){
  ...
  var selectedDay:LocalDate by androidx.compose.runtime.mutableStateOf(LocalDate.now());private set
  var day:LoadState<DayPage> by androidx.compose.runtime.mutableStateOf(LoadState.Loading);private set
  var memories:LoadState<List<DayEntry>> by androidx.compose.runtime.mutableStateOf(LoadState.Loading);private set
  ...
  var projectActionSubmit:SubmitState by androidx.compose.runtime.mutableStateOf(SubmitState.Editing);private set
  var moneyPlanningSubmit:SubmitState by androidx.compose.runtime.mutableStateOf(SubmitState.Editing);private set
  var moneyImportMonth:LoadState<MoneyImportMonthSummary> by androidx.compose.runtime.mutableStateOf(LoadState.Loading);private set
  var moneyImportReview:LoadState<MoneyImportReviewSummary>? by androidx.compose.runtime.mutableStateOf(null);private set
  var moneyImportSubmit:SubmitState by androidx.compose.runtime.mutableStateOf(SubmitState.Editing);private set
  var offlineItinerary:LoadState<NativeOfflineItinerary>? by androidx.compose.runtime.mutableStateOf(null);private set
  ...
  var assistantThreads:LoadState<List<AssistantThreadSummary>> by androidx.compose.runtime.mutableStateOf(LoadState.Loading);private set
  ...
  val currentAssistantThreadId:String? get()=assistantThreadId
  ...
  fun activateAccount(accountId:String){if(activeAccountId==accountId)return;invalidateReads();landingJob?.cancel();commitRefreshJob?.cancel();pendingRefreshCapabilities.clear();assetPreviews.clear();activeAccountId=accountId;assistantThreadId=null;assistant=null;assistantHistory=null;assistantThreads=LoadState.Loading;searchResults=null;workspaceDomain=null;selectedTravelTripId=null;activeDetail=null;today=LoadState.Loading;homeHealth=LoadState.Loading;timeline=LoadState.Loading;day=LoadState.Loading;memories=LoadState.Loading;plans=LoadState.Loading;library=LoadState.Loading;projectLinks=LoadState.Loading;inbox=LoadState.Loading;workspace=LoadState.Loading;workspaceOverview=LoadState.Loading;detail=LoadState.Loading;planDetail=LoadState.Loading;ownedItemDetail=LoadState.Loading;reviewDetail=LoadState.Loading;consumptionStats=LoadState.Loading;releaseHistory=LoadState.Loading;submit=SubmitState.Editing;mealPlanningSubmit=SubmitState.Editing;projectActionSubmit=SubmitState.Editing;moneyPlanningSubmit=SubmitState.Editing;moneyImportMonth=LoadState.Loading;moneyImportReview=null;offlineItinerary=null;moneyImportSubmit=SubmitState.Editing;serviceCards=LoadState.Loading;selectedServiceCard=null;serviceCardSubmit=SubmitState.Editing;serviceCardQuery="";observeQueue();observeDeviceSync(accountId);refreshToday();landingJob=viewModelScope.launch{delay(450);if(activeAccountId==accountId)refreshHomeHealth()}}
  fun deactivateAccount(){invalidateReads();assetPreviews.clear();queueJob?.cancel();deviceSyncJob?.cancel();landingJob?.cancel();commitRefreshJob?.cancel();pendingRefreshCapabilities.clear();workspaceJob?.cancel();workspaceOverviewJob?.cancel();queueJob=null;deviceSyncJob=null;landingJob=null;commitRefreshJob=null;workspaceJob=null;workspaceOverviewJob=null;activeAccountId=null;assistantThreadId=null;assistant=null;assistantHistory=null;assistantThreads=LoadState.Loading;searchResults=null;workspaceDomain=null;selectedTravelTripId=null;activeDetail=null;today=LoadState.Loading;homeHealth=LoadState.Loading;timeline=LoadState.Loading;day=LoadState.Loading;memories=LoadState.Loading;plans=LoadState.Loading;library=LoadState.Loading;workspace=LoadState.Loading;workspaceOverview=LoadState.Loading;refundCandidates=LoadState.Loading;detail=LoadState.Loading;planDetail=LoadState.Loading;ownedItemDetail=LoadState.Loading;reviewDetail=LoadState.Loading;consumptionStats=LoadState.Loading;releaseHistory=LoadState.Loading;projectLinks=LoadState.Loading;queueStatus=LoadState.Loading;deviceSyncStatus=DeviceSyncStatus();inbox=LoadState.Loading;submit=SubmitState.Editing;mealPlanningSubmit=SubmitState.Editing;projectActionSubmit=SubmitState.Editing;moneyPlanningSubmit=SubmitState.Editing;moneyImportMonth=LoadState.Loading;moneyImportReview=null;offlineItinerary=null;moneyImportSubmit=SubmitState.Editing;serviceCards=LoadState.Loading;selectedServiceCard=null;serviceCardSubmit=SubmitState.Editing;serviceCardQuery=""}
  ...
  fun loadDay(date:LocalDate=selectedDay){selectedDay=date;val token=beginRead("day");day=LoadState.Loading;memories=LoadState.Loading;viewModelScope.launch{val result=load("这一天没有记录"){repository.day(date)};if(isCurrentRead("day",token))day=result};viewModelScope.launch{val result=load("往年同日没有记录"){repository.memories(date)};if(isCurrentRead("day",token))memories=result}}
  fun loadMoreDay(){val current=(day as? LoadState.Ready)?.value?:return;val cursor=current.nextCursor?:return;val token=beginRead("day");viewModelScope.launch{val next=load("没有更多记录"){repository.day(selectedDay,cursor)};if(isCurrentRead("day",token)&&next is LoadState.Ready)day=LoadState.Ready(current.copy(items=(current.items+next.value.items).distinctBy{"${it.domain}:${it.kind}:${it.id}"},nextCursor=next.value.nextCursor,total=next.value.total))}}
  ...
  fun refreshAssistantThreads(){val token=beginRead("assistant-threads");viewModelScope.launch{val result=loadList("还没有对话"){repository.recentThreads()};if(isCurrentRead("assistant-threads",token))assistantThreads=result}}
  fun openAssistant(){refreshAssistantThreads();if(assistantHistory is LoadState.Loading||assistantHistory is LoadState.Ready||assistantHistory is LoadState.Empty)return;val token=beginRead("assistant-history");assistantHistory=LoadState.Loading;viewModelScope.launch{val result=try{val threadId=assistantThreadId?:repository.recentThreads().firstOrNull()?.id;if(isCurrentRead("assistant-history",token))assistantThreadId=threadId;if(threadId==null)LoadState.Empty("开始一段新对话") else LoadState.Ready(repository.assistantMessages(threadId))}catch(error:CancellationException){throw error}catch(error:Exception){LoadState.Failed(error.message?:"无法读取对话")};if(isCurrentRead("assistant-history",token))assistantHistory=result}}
  fun selectAssistantThread(id:String){if(assistant is LoadState.Loading)return;assistantThreadId=id;assistant=null;val token=beginRead("assistant-history");assistantHistory=LoadState.Loading;viewModelScope.launch{val result=load("对话不存在"){repository.assistantMessages(id)};if(isCurrentRead("assistant-history",token)&&assistantThreadId==id)assistantHistory=result}}
  fun startNewAssistantThread(){if(assistant is LoadState.Loading)return;beginRead("assistant-history");assistantThreadId=null;assistant=null;assistantHistory=LoadState.Empty("开始一段新对话")}
  ...
  fun askLife(message:String){if(message.isBlank()||assistant is LoadState.Loading)return;val token=beginRead("assistant-run");val threadId=assistantThreadId;assistant=LoadState.Loading;viewModelScope.launch{val result=try{val reply=repository.assist(message,threadId);val history=repository.assistantMessages(reply.threadId);if(isCurrentRead("assistant-run",token)){beginRead("assistant-history");assistantThreadId=reply.threadId;assistantHistory=LoadState.Ready(history);refreshAssistantThreads();if(reply.receipts.isNotEmpty())refreshForCapabilities(reply.receipts.map{it.capability}.toSet())};LoadState.Ready(reply)}catch(error:CancellationException){throw error}catch(error:Exception){LoadState.Failed(error.message?:"Life 请求失败")};if(isCurrentRead("assistant-run",token))assistant=result}}
  ...
  fun saveTravelChecklist(tripId:String,revision:Int?,items:List<TravelChecklistItemSummary>){if(submit is SubmitState.Sending)return;submit=SubmitState.Sending("travel_checklist");viewModelScope.launch{try{submit=SubmitState.Saved(repository.saveTravelChecklist(tripId,revision,items))}catch(error:CancellationException){throw error}catch(error:Exception){submit=SubmitState.Rejected(error.message?:"无法保存清单")}}}
  fun prepareOfflineItinerary(tripId:String){val token=beginRead("offline-itinerary");offlineItinerary=LoadState.Loading;viewModelScope.launch{val result=load("无法生成行程单"){repository.prepareOfflineItinerary(tripId)};if(isCurrentRead("offline-itinerary",token))offlineItinerary=result}}
  fun loadMoneyImportMonth(period:String=LocalDate.now().toString().take(7)){val token=beginRead("money-import-month");moneyImportMonth=LoadState.Loading;viewModelScope.launch{val result=load("这个月没有账单候选"){repository.moneyImportMonth(period)};if(isCurrentRead("money-import-month",token))moneyImportMonth=result}}
  fun openMoneyImportBatch(id:String){val token=beginRead("money-import-review");moneyImportReview=LoadState.Loading;viewModelScope.launch{val result=load("批次已不可用"){repository.moneyImportReview(id)};if(isCurrentRead("money-import-review",token))moneyImportReview=result}}
  fun resolveMoneyImport(candidate:MoneyImportCandidateSummary,decision:String,entryType:String,amount:String,date:String,category:String,originalEntryId:String){if(moneyImportSubmit is SubmitState.Sending)return;moneyImportSubmit=SubmitState.Sending("money_import");viewModelScope.launch{try{moneyImportSubmit=SubmitState.Saved(repository.resolveMoneyImport(candidate,decision,entryType,amount,date,category,originalEntryId))}catch(error:CancellationException){throw error}catch(error:Exception){moneyImportSubmit=SubmitState.Rejected(error.message?:"账单复核失败")}}}
  ...
  fun saveImportedRecipe(title:String,servings:String,ingredients:String,instructions:String,sourceUrl:String)=mealPlanningWrite{repository.saveImportedRecipe(title,servings,ingredients,instructions,sourceUrl)}
  fun saveProjectAction(projectId:String,title:String,date:String,time:String){if(projectActionSubmit is SubmitState.Sending)return;projectActionSubmit=SubmitState.Sending("project_action");viewModelScope.launch{try{projectActionSubmit=SubmitState.Saved(repository.saveProjectAction(projectId,title,date,time))}catch(error:CancellationException){throw error}catch(error:Exception){projectActionSubmit=SubmitState.Rejected(error.message?:"行动保存失败")}}}
  fun setFoodStock(lot:MealPlanningResultDtoStockLotsEntry?,name:String,quantity:String,unit:String,expiresOn:String?)=mealPlanningWrite{repository.setFoodStock(lot,name,quantity,unit,expiresOn)}
  ...
  fun queueLibraryVision(itemId:String,assetVersionId:String){if(submit is SubmitState.Sending)return;submit=SubmitState.Sending("cmd_pending");viewModelScope.launch{try{submit=SubmitState.Saved(repository.queueLibraryVision(itemId,assetVersionId))}catch(error:CancellationException){throw error}catch(error:Exception){submit=SubmitState.Rejected(error.message?:"视觉处理排队失败")}}}
  fun retryLibraryVision(jobId:String){if(submit is SubmitState.Sending)return;submit=SubmitState.Sending("cmd_pending");viewModelScope.launch{try{submit=SubmitState.Saved(repository.retryLibraryVision(jobId))}catch(error:CancellationException){throw error}catch(error:Exception){submit=SubmitState.Rejected(error.message?:"视觉处理重试失败")}}}
  ...
  private fun refreshForCapabilities(capabilities:Set<String>){
    ...
    refreshToday();refreshTimeline();loadDay()
    ...
    if(capabilities.contains("money.resolve_import_candidate")){loadMoneyImportMonth((moneyImportMonth as? LoadState.Ready)?.value?.period?:LocalDate.now().toString().take(7));(moneyImportReview as? LoadState.Ready)?.value?.batchId?.let(::openMoneyImportBatch)}
    if(capabilities.any{it.startsWith("life.save_project")||it.startsWith("life.save_action")||it.startsWith("life.save_owned")||it.startsWith("life.record_owned")||it.startsWith("life.generate_review")||it.startsWith("life.save_meal")||it.startsWith("life.build_shopping")||it.startsWith("life.update_shopping")||it.startsWith("life.set_food_stock")||it.startsWith("money.set_")||it.startsWith("health.set_plan")})refreshPlans()
