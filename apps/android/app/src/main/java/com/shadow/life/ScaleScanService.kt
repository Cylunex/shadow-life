...
class ScaleScanService:Service(){
  ...
  private var scanner:BluetoothLeScanner?=null;private var callback:ScanCallback?=null;private var accountId:String?=null
  ...
  private val finish=Runnable{update("没有收到稳定读数，请重新开秤");stopSelf()}
  ...
  override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{
    accountId=intent?.getStringExtra(EXTRA_ACCOUNT_ID);startForeground(NOTIFICATION_ID,notification("三分钟内上秤即可记录"));handler.removeCallbacks(finish);handler.postDelayed(finish,TIMEOUT_MS);restartScan();return START_NOT_STICKY
  }
  override fun onDestroy(){handler.removeCallbacksAndMessages(null);stopScan();scope.cancel();super.onDestroy()}

  private fun restartScan(){stopScan();val manager=getSystemService(BluetoothManager::class.java);val adapter=manager?.adapter;if(adapter==null||!adapter.isEnabled){update("蓝牙未开启");return};scanner=adapter.bluetoothLeScanner;if(scanner==null){update("蓝牙不可用");return}
    val active=object:ScanCallback(){
      override fun onScanResult(callbackType:Int,result:ScanResult){val record=result.scanRecord?:return;val address=result.device?.address?:"unknown";record.getServiceData(bodyUuid)?.let{data->handler.post{XiaomiScaleParser.parseScale2(data)?.let{accept(address,it)}}};record.getServiceData(miBeaconUuid)?.let{data->handler.post{acceptS400(address,data)}}}
      override fun onScanFailed(errorCode:Int){update("扫描失败（$errorCode），请重试");callback=null;scanner=null}
    };callback=active
    ...
    try{scanner?.startScan(filters,settings,active)}catch(_:Exception){try{scanner?.startScan(null,settings,active)}catch(_:Exception){callback=null;scanner=null;update("没有蓝牙扫描权限")}}
  }
  ...
  private fun acceptS400(address:String,data:ByteArray){val prefs=runCatching{ScalePreferences(this).current()}.getOrNull();val frame=XiaomiScaleParser.parseS400(data,address,prefs?.s400Bindkey)
    if(frame==null){val now=System.currentTimeMillis();if(XiaomiScaleParser.isS400(data)&&now-lastBindkeyWarningMs>30_000){lastBindkeyWarningMs=now;update("检测到 S400，请先配置正确的 bindkey")};return}
  ...
  private fun enqueue(measurement:Measurement){val now=System.currentTimeMillis();sent.entries.removeIf{now-it.value>10*60_000};if(sent.putIfAbsent(measurement.key,now)!=null)return;val weight=measurement.frame.weightKg?:return;update("已收到稳定读数，正在安全保存…")
    scope.launch{val app=application as ShadowApp;val session=app.sessions.active()?.takeIf{it.accountId==accountId};if(session==null){update("登录已失效，读数未保存");return@launch}
      ...
      try{app.queue.enqueueCommand(session,command.commandId,command.capability,command.body);SyncScheduler.schedule(this@ScaleScanService,session.accountId);update("已保存 ${decimal(weight)} kg${if(composition!=null)"，含体成分" else if(impedance!=null)"，请补档案以计算体成分" else ""}");handler.postDelayed({stopSelf()},4_000)}catch(_:Exception){update("读数保存失败，请重试")}
    }
  }
  ...
  companion object{const val EXTRA_ACCOUNT_ID="account_id";private const val CHANNEL="shadow-life-scale";private const val NOTIFICATION_ID=1201;private const val TIMEOUT_MS=3*60_000L;private const val SETTLE_MS=12_000L}
}
