...
object SamsungSync {
  ...
  @JvmStatic fun enable(activity:Activity,accountId:String){scope.launch{try{val store=HealthDataService.getStore(activity.applicationContext);var granted=store.getGrantedPermissions(PERMISSIONS);if(!granted.containsAll(PERMISSIONS))granted=store.requestPermissions(PERMISSIONS,activity);if(granted.isEmpty()){Toast.makeText(activity,"未授予 Samsung Health 读取权限",Toast.LENGTH_LONG).show();return@launch};schedule(activity,accountId);Toast.makeText(activity,"Samsung Health 同步已启动",Toast.LENGTH_SHORT).show()}catch(error:Exception){Log.w("SamsungSync","Samsung Health connection failed",error);Toast.makeText(activity,"Samsung Health 连接失败：${error.message}",Toast.LENGTH_LONG).show()}}}
  private fun schedule(context:Context,accountId:String){val data=workDataOf("account_id" to accountId);val constraints=Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build();WorkManager.getInstance(context).enqueueUniquePeriodicWork("shadow-samsung-$accountId",ExistingPeriodicWorkPolicy.UPDATE,PeriodicWorkRequestBuilder<SamsungSyncWorker>(1,TimeUnit.HOURS).setInputData(data).setConstraints(constraints).build());WorkManager.getInstance(context).enqueue(OneTimeWorkRequestBuilder<SamsungSyncWorker>().setInputData(data).setConstraints(constraints).build())}
}

class SamsungSyncWorker(context:Context,params:WorkerParameters):CoroutineWorker(context,params){
  override suspend fun doWork():Result {
    val accountId=inputData.getString("account_id")?:return Result.failure();val app=applicationContext as ShadowApp;val session=app.sessions.active()?.takeIf{it.accountId==accountId}?:return Result.failure()
    return try{val store=HealthDataService.getStore(applicationContext);val granted=store.getGrantedPermissions(SamsungSync.PERMISSIONS);if(granted.isEmpty())return Result.failure(workDataOf("reason" to "permissions_required"));val now=System.currentTimeMillis();val prefs=applicationContext.getSharedPreferences("samsung-health-sync",Context.MODE_PRIVATE);val last=prefs.getLong("last_sync_$accountId",0);val days=if(last==0L)7 else (((now-last)/86_400_000L).toInt()+2).coerceIn(2,30);val end=LocalDate.now();val start=end.minusDays((days-1).toLong());val healthConnectPermissions=if(HealthConnectSync.available(applicationContext))HealthConnectClient.getOrCreate(applicationContext).permissionController.getGrantedPermissions() else emptySet();val commands=readCommands(store,session,start,end,now,granted,healthConnectPermissions);commands.forEach{app.queue.enqueueCommand(session,it.commandId,it.capability,it.body)};if(commands.isNotEmpty())SyncScheduler.schedule(applicationContext,accountId);prefs.edit().putLong("last_sync_$accountId",now).apply();Result.success(workDataOf("records" to commands.size))}catch(error:Exception){Log.w("SamsungSyncWorker","Samsung read failed",error);Result.retry()}
  }
