...
class SamsungSyncWorker(context:Context,params:WorkerParameters):CoroutineWorker(context,params){
  ...
  private suspend fun readCommands(store:HealthDataStore,session:ProductSession,start:LocalDate,end:LocalDate,version:Long,samsungPermissions:Set<Permission>,healthConnectPermissions:Set<String>):List<HealthQueuedCommand>{
    ...
    fun add(type:String,id:String,payload:JSONObject){out+=healthDeviceRecordCommand(session.accountId,session.subjectId,"samsung",instance,fingerprint,type,id,version,payload,"samsung-data-3")}
    ...
    if(Permission.of(DataTypes.STEPS,AccessType.READ) in samsungPermissions && HealthPermission.getReadPermission(StepsRecord::class) !in healthConnectPermissions)runCatching{val request=DataType.StepsType.TOTAL.requestBuilder.setLocalTimeFilterWithGroup(timeFilter,LocalTimeGroup.of(LocalTimeGroupUnit.DAILY,1)).build();store.aggregateData(request).dataList.forEach{point->point.value?.takeIf{it>0}?.let{steps->val payload=activityPayload(point.getStartLocalDateTime().toLocalDate());payload.put("steps",steps);payload.getJSONObject("field_sources").put("steps","samsung_data_sdk")}}}.onFailure{Log.w("SamsungSyncWorker","steps read failed",it)}
    if(Permission.of(DataTypes.ACTIVITY_SUMMARY,AccessType.READ) in samsungPermissions)runCatching{
      val group=LocalTimeGroup.of(LocalTimeGroupUnit.DAILY,1)
      store.aggregateData(DataType.ActivitySummaryType.TOTAL_ACTIVE_TIME.requestBuilder.setLocalTimeFilterWithGroup(timeFilter,group).build()).dataList.forEach{point->point.value?.toMinutes()?.takeIf{it>0}?.let{minutes->val payload=activityPayload(point.getStartLocalDateTime().toLocalDate());payload.put("active_minutes",minutes);payload.getJSONObject("field_sources").put("active_minutes","samsung_data_sdk")}}
      store.aggregateData(DataType.ActivitySummaryType.TOTAL_ACTIVE_CALORIES_BURNED.requestBuilder.setLocalTimeFilterWithGroup(timeFilter,group).build()).dataList.forEach{point->point.value?.takeIf{it>0}?.let{calories->val payload=activityPayload(point.getStartLocalDateTime().toLocalDate());payload.put("device_calories_kcal",decimal(calories.toDouble()));payload.getJSONObject("field_sources").put("device_calories_kcal","samsung_data_sdk")}}
    }.onFailure{Log.w("SamsungSyncWorker","activity summary read failed",it)}
    activity.toSortedMap().forEach{(day,payload)->add("daily_activity","samsung-steps-$day",payload)}
