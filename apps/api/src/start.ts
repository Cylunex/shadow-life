...
function projectLinks(value:string|undefined):ProjectDirectoryResult{if(value===undefined)return projectDirectoryResultSchema.parse({items:[{id:"foliant",title:"Shadow Foliant",subtitle:"股票研究",launch_mode:"app_link",app_link_url:null,web_fallback_url:null,android_package:null,state:"missing_configuration"},{id:"garden",title:"Shadow Garden",subtitle:"博客创作",launch_mode:"browser",app_link_url:null,web_fallback_url:null,android_package:null,state:"missing_configuration"}]});let parsed:unknown;try{parsed=JSON.parse(value);}catch{throw new Error("SHADOW_PROJECT_LINKS must be valid JSON");}return projectDirectoryResultSchema.parse({items:parsed});}
const configuredProjectLinks=projectLinks(process.env.SHADOW_PROJECT_LINKS);
