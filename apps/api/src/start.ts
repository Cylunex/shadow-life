...
import { supportedLifeEffects } from "./auth.js";
import { loadProjectLinks } from "./project-links.js";
...
const clientSubjects=identitySubjects(process.env.SHADOW_OIDC_CLIENT_SUBJECTS);
function oidcClientEffects(value:string|undefined):Record<string,ReadonlySet<string>>|undefined{if(!value)return undefined;const parsed=JSON.parse(value) as unknown;if(!parsed||typeof parsed!=="object"||Array.isArray(parsed))throw new Error("SHADOW_OIDC_CLIENT_EFFECTS must be a JSON object");const entries=Object.entries(parsed as Record<string,unknown>);if(entries.some(([clientId,effects])=>!clientId||!Array.isArray(effects)||effects.length===0||effects.some(effect=>typeof effect!=="string"||!supportedLifeEffects.has(effect))))throw new Error("SHADOW_OIDC_CLIENT_EFFECTS contains an invalid client policy");return Object.fromEntries(entries.map(([clientId,effects])=>[clientId,new Set(effects as string[])]));}
const clientEffects=oidcClientEffects(process.env.SHADOW_OIDC_CLIENT_EFFECTS);
const auth=process.env.SHADOW_OIDC_ISSUER&&process.env.SHADOW_OIDC_AUDIENCE&&process.env.SHADOW_OIDC_JWKS_URL?{issuer:process.env.SHADOW_OIDC_ISSUER,audience:process.env.SHADOW_OIDC_AUDIENCE,jwksUrl:process.env.SHADOW_OIDC_JWKS_URL,environmentId:process.env.SHADOW_ENVIRONMENT_ID??"default",...(process.env.SHADOW_OIDC_ALLOWED_CLIENTS?{allowedClients:process.env.SHADOW_OIDC_ALLOWED_CLIENTS.split(",").map(value=>value.trim()).filter(Boolean)}:{}),...(subjectMap?{identitySubjects:subjectMap}:{}),...(clientSubjects?{clientSubjects}:{}),...(clientEffects?{clientEffects}:{}),...(process.env.SHADOW_WEB_ORIGIN?{webOrigin:process.env.SHADOW_WEB_ORIGIN}:{})}:undefined;
...
const configuredProjectLinks=loadProjectLinks(
  process.env.SHADOW_PROJECT_LINKS,
  process.env.SHADOW_PROJECT_LINKS_FILE,
  {
    foliantUrl: process.env.SHADOW_FOLIANT_URL,
    gardenUrl: process.env.SHADOW_GARDEN_URL
  }
);
if(process.env.NODE_ENV==="production"&&!proxyAuth&&(!auth||!webSession||(!subjectMap&&!clientSubjects)||!auth.allowedClients?.length))throw new Error("Production requires complete OIDC, client allowlist and subject mapping settings or trusted proxy auth settings");
