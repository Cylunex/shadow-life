...
const capabilities=(names:string[])=>({subject_id:"subject_test",client_id:"client_test",issuer:"https://identity.example.com",capabilities:names.map(name=>({name}))});

test("dashboard loads only authorized domains and keeps sibling successes",async()=>{
  const calls:string[]=[];const fetcher=async(input:RequestInfo|URL)=>{const url=String(input);calls.push(url);if(url==="/api/write-epochs")return response({items:[{domain:"health",epoch:2}]});if(url==="/api/capabilities")return response(capabilities(["money.records","health.records"]));if(url==="/api/money")return response({items:[{id:"money_1"}]});if(url==="/api/health")return response({message:"denied"},403);throw new Error(`unexpected ${url}`);};
...
test("a missing daily summary is an empty state while other health reads remain usable",async()=>{
  const fetcher=async(input:RequestInfo|URL)=>{const url=String(input);if(url==="/api/write-epochs")return response({items:[]});if(url==="/api/capabilities")return response(capabilities(["health.daily","health.trend"]));if(url.startsWith("/api/health/daily/"))return response({message:"not found"},404);if(url.startsWith("/api/health/trend?"))return response({metric_key:"weight",points:[],coverage:{from:null,to:null,points:0,truncated:false},as_of:"2026-09-10T01:00:00Z"});throw new Error(`unexpected ${url}`);};
