...
import { buildWorkoutCommand, initialWorkoutFields } from "./workout.js";
...
export function WorkoutPanel({records,date,timeZone,execute,onChanged}:{records:unknown[];date:string;timeZone:string;execute:Execute;onChanged:()=>Promise<void>}){
  ...
  return <section className="workout-panel">
    ...
    <h3>最近训练</h3>{sessions.length?<ul className="workout-history">{sessions.map(session=><li key={String(session.id)}><b>{String(session.sessionType??session.session_type??"训练")}</b><span>{String(session.occurredOn??session.occurred_on??"")}{session.durationMinutes??session.duration_minutes?` · ${String(session.durationMinutes??session.duration_minutes)} 分钟`:""}{session.planId??session.plan_id?" · 来自计划":""}</span></li>)}</ul>:<p>还没有实际训练记录。</p>}
  </section>;
}
