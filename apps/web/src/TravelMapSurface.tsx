...
export function TravelMapSurface({places,tracks,dayPlan,allPlans}:{places:readonly TravelPlace[];tracks:readonly TravelTrack[];dayPlan?:TravelDayPlan|undefined;allPlans?:readonly TravelDayPlan[]|undefined}){
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(()=>{
      ...
      mapRef.current=new google.maps.Map(containerRef.current,{center:initialCenter(places),zoom:places.length?13:4,mapTypeControl:false,streetViewControl:false,fullscreenControl:false,gestureHandling:"greedy"});
      setStatus("ready");
