...
export function TravelMapItinerary({places,plans,selectedDate,scope,initiallyOpen}:{places:readonly TravelPlace[];plans:readonly TravelDayPlan[];selectedDate:string|undefined;scope:"day"|"all";initiallyOpen?:boolean}){
  const visible=scope==="day"?plans.filter(plan=>plan.plan_date===selectedDate):plans;
  const itineraryKey=JSON.stringify([scope,selectedDate,visible.map(plan=>[plan.id,plan.items.map(stop=>[stop.stop_id,stop.place_id,stop.title,stop.note])])]);
  ...
  return <details className="travel-map-itinerary" open={initiallyOpen??scope==="day"}>
    <summary>{scope==="day"?"当天地点":"全程地点"} · {count} 个可定位停留点 <span>展开地点与路线</span></summary>
    <div key={itineraryKey} className="travel-map-stop-scroll">
