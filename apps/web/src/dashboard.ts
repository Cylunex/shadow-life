...
export async function loadDashboard(fetcher:typeof fetch,headers:HeadersInit,options:{date?:string;timeZone?:string}={}):Promise<DashboardLoad>{
  ...
  let today:TodayOverview|undefined,timeline:DashboardLoad["timeline"],daily:Record<string,unknown>|undefined,trend:HealthTrend|undefined,sources:HealthSource[]|undefined;const healthErrors:HealthWorkspace["errors"]={};
  await Promise.all([
    ...
    visible.has("health.daily")?(async()=>{try{const response=await fetcher(`/api/health/daily/${encodeURIComponent(date)}`,{headers});if(response.status===404)return;if(!response.ok)throw new Error(`HTTP ${response.status}`);daily=await response.json() as Record<string,unknown>;}catch(error){healthErrors.daily=error instanceof Error?error.message:"读取失败";}})():Promise.resolve(),
    visible.has("health.trend")?(async()=>{try{trend=await loadHealthTrend(fetcher,headers,"weight");}catch(error){healthErrors.trend=error instanceof Error?error.message:"读取失败";}})():Promise.resolve(),
