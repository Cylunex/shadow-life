...
test("vision worker preserves original and writes a correctable sourced revision",pgOnly,async t=>{
  ...
  assert.match(detail.snippets[0]!.text,/合成车票识别词/u);assert.equal(detail.revisions[0]!.text,"一张车票");assert.equal(detail.revisions[0]!.source_processing_job_id,queued.actual_values.library_processing_job_id);
  assert.equal(detail.processing_jobs[0]!.suggestion?.document_date,"2026-09-10");
  await f.run("library.revise",{item_id:itemId,expected_revision:2,title:"合成车票",text:"人工核对的正文",tags:[],document_date:"2026-09-10",category:"旅行",reason:"用户更正识别摘要"});
  const confirmed=await f.queries.libraryItem(f.context,itemId);assert.equal(confirmed.revisions[0]!.text,"人工核对的正文");assert.equal(confirmed.revisions[1]!.source_processing_job_id,queued.actual_values.library_processing_job_id);
...
test("unconfigured vision fails visibly and retries through the Executor",pgOnly,async t=>{
  ...
  detail=await f.queries.libraryItem(f.context,itemId);assert.equal(detail.processing_jobs[0]!.attempts,2);assert.equal(detail.revisions[0]!.text,"人工核对");
});
