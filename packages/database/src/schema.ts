import { bigint, boolean, check, date, index, integer, jsonb, numeric, pgTable, primaryKey, text, time, timestamp, uniqueIndex } from "drizzle-orm/pg-core";
...
export const recurringPlans = pgTable("recurring_plans", { id: text("id").primaryKey(), subjectId: text("subject_id").references(() => principals.id).notNull(), title: text("title").notNull(), amount: numeric("amount", { precision: 24, scale: 6 }), currency: text("currency").notNull(), cadence: text("cadence").notNull(), nextDueOn: date("next_due_on").notNull(), anchorOn:date("anchor_on").notNull(), localTime:time("local_time"), missingDatePolicy:text("missing_date_policy").notNull(), recurrenceRule:text("recurrence_rule").notNull(), category: text("category"), active: boolean("active").notNull(), timeZone:text("time_zone").notNull(), intervalDays:integer("interval_days"), state:text("state").notNull(), endedOn:date("ended_on"), revision: integer("revision").default(1).notNull() });
...
export const tripRevisions=pgTable("trip_revisions",{tripId:text("trip_id").references(()=>trips.id).notNull(),revision:integer("revision").notNull(),snapshot:jsonb("snapshot").notNull(),reason:text("reason").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull()},table=>[primaryKey({columns:[table.tripId,table.revision]})]);
export const tripPlanVersions=pgTable("trip_plan_versions",{id:text("id").primaryKey(),tripId:text("trip_id").references(()=>trips.id,{onDelete:"cascade"}).notNull(),subjectId:text("subject_id").references(()=>principals.id).notNull(),version:integer("version").notNull(),label:text("label"),note:text("note"),snapshot:jsonb("snapshot").notNull(),contentHash:text("content_hash").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).defaultNow().notNull()},table=>[uniqueIndex("trip_plan_versions_trip_version_unique").on(table.tripId,table.version)]);
export const tripRuns=pgTable("trip_runs",{id:text("id").primaryKey(),tripId:text("trip_id").references(()=>trips.id,{onDelete:"cascade"}).notNull(),subjectId:text("subject_id").references(()=>principals.id).notNull(),planVersionId:text("plan_version_id").references(()=>tripPlanVersions.id).notNull(),state:text("state").notNull(),startedAt:timestamp("started_at",{withTimezone:true}).defaultNow().notNull(),completedAt:timestamp("completed_at",{withTimezone:true})},table=>[uniqueIndex("trip_runs_one_active_per_member").on(table.tripId,table.subjectId).where(sql`${table.state}='active'`)]);
export const tripStopOutcomes=pgTable("trip_stop_outcomes",{runId:text("run_id").references(()=>tripRuns.id).notNull(),subjectId:text("subject_id").references(()=>principals.id).notNull(),stopId:text("stop_id").notNull(),state:text("state").notNull(),occurredAt:timestamp("occurred_at",{withTimezone:true}),note:text("note"),revision:integer("revision").default(1).notNull(),updatedAt:timestamp("updated_at",{withTimezone:true}).defaultNow().notNull()},table=>[primaryKey({columns:[table.runId,table.stopId]})]);
...
export const libraryItems = pgTable("library_items", { id: text("id").primaryKey(), subjectId: text("subject_id").references(() => principals.id).notNull(), title: text("title").notNull(), itemType: text("item_type").notNull(), state:text("state").default("active").notNull(), currentRevision: integer("current_revision").default(1).notNull(), sourceId: text("source_id").references(() => sources.id), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull() });
