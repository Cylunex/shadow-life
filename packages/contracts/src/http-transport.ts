...
export const queryTransportRegistry:Readonly<Record<string,ReadRoute>>={
  ...
  "life.day":input=>get(`/api/life/day${search({date:input.date,time_zone:input.time_zone,limit:input.limit,cursor:input.cursor})}`),
  "life.memories":input=>get(`/api/life/memories${search({date:input.date,time_zone:input.time_zone})}`),
  ...
  "health.workout_progression":input=>get(`/api/health/workout-progression${search({plan_id:input.plan_id})}`),
