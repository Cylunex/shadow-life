...
import { agentContextPackInputSchema, agentMemoriesInputSchema, createAgentContextPackInputSchema, notificationsInputSchema, revokeAgentContextPackInputSchema, setAgentMemoryInputSchema, setNotificationPreferencesInputSchema, updateNotificationInputSchema } from "./schemas.js";
import { generateLifeReviewInputSchema, lifeReviewsInputSchema, ownedItemsInputSchema, recordOwnedItemEventInputSchema, saveOwnedItemInputSchema } from "./schemas.js";
...
function contextEffects(value:{object_refs?:readonly {kind:string}[];evidence_refs?:readonly {kind:string}[]}):string[]{const refs=value.object_refs??value.evidence_refs??[],effects=new Set<string>(["agent.run"]);for(const ref of refs){if(ref.kind==="meal"||ref.kind==="purchase")effects.add("life.meal.read");else if(ref.kind==="money_entry")effects.add("money.entry.read");else if(ref.kind==="health_measurement"||ref.kind==="health_workout")effects.add("health.measurement.read");else if(ref.kind==="trip")effects.add("travel.trip.read");else if(ref.kind==="library_item")effects.add("library.item.read");}return[...effects].sort();}
const reviewDomainEffect={money:"money.entry.read",meals:"life.meal.read",health:"health.measurement.read",items:"life.item.read",library:"library.item.read"} as const;
...
export const capabilityRegistry = {
  ...
  "agent.create_context_pack":{...write("agent.create_context_pack","创建最多一小时有效的授权对象引用包；只保存身份、版本与时间范围，不复制事实正文。",createAgentContextPackInputSchema,["agent.run","life.meal.read","money.entry.read","health.measurement.read","travel.trip.read","library.item.read"]),resolveEffects(input:unknown){return contextEffects(createAgentContextPackInputSchema.parse(input));}},
  "agent.revoke_context_pack":write("agent.revoke_context_pack","立即撤销本人短期对象上下文包。",revokeAgentContextPackInputSchema,["agent.run"]),
  "agent.set_memory":{...write("agent.set_memory","保存显式偏好或有算法和证据的确定性聚合；模型推断不属于可持久化类别。",setAgentMemoryInputSchema,["agent.run","life.meal.read","money.entry.read","health.measurement.read","travel.trip.read","library.item.read"]),resolveEffects(input:unknown){return contextEffects(setAgentMemoryInputSchema.parse(input));}},
  "notifications.set_preferences":write("notifications.set_preferences","设置通知总开关、时区和可跨午夜的静默时间。",setNotificationPreferencesInputSchema,["notifications.write"]),
  "notifications.update":write("notifications.update","忽略、延期或标记本人通知已投递。",updateNotificationInputSchema,["notifications.write"]),
  "life.save_owned_item":{...write("life.save_owned_item","显式创建或更新本人持有物品，可关联一条购买明细和精确版本的资料凭证；不会从购买记录自动生成库存。",saveOwnedItemInputSchema,["life.item.write","life.meal.read","library.item.read"]),resolveEffects(input:unknown){const value=saveOwnedItemInputSchema.parse(input);return["life.item.write",...(value.purchase_item_id?["life.meal.read"]:[]),...(value.documents.length?["library.item.read"]:[])];}},
  "life.record_owned_item_event":{...write("life.record_owned_item_event","记录维护、维修、退货、赠予、丢失或处置事件，并按事件显式推进物品状态。",recordOwnedItemEventInputSchema,["life.item.write","money.entry.read","library.item.read"]),resolveEffects(input:unknown){const value=recordOwnedItemEventInputSchema.parse(input);return["life.item.write",...(value.cost_entry_id?["money.entry.read"]:[]),...(value.document?["library.item.read"]:[])];}},
  "life.generate_review":{...write("life.generate_review","用固定算法重算指定期间的跨领域指标、覆盖率和有界证据集合；不生成生活总分或健康因果结论。",generateLifeReviewInputSchema,["life.review.write","money.entry.read","life.meal.read","health.measurement.read","life.item.read","library.item.read"]),resolveEffects(input:unknown){const value=generateLifeReviewInputSchema.parse(input);return["life.review.write",...value.domains.map(domain=>reviewDomainEffect[domain])];}},
  ...
  "agent.get_context_pack":{name:"agent.get_context_pack",description:"读取仍有效且当前权限仍覆盖全部引用的短期上下文包。",inputSchema:agentContextPackInputSchema,commandSchema:agentContextPackInputSchema,resultSchema:resourceDetailResultSchema,possibleEffects:["agent.run"] as const,idempotency:"not-applicable" as const,statusQuery:"operations.get",resolveEffects:()=>["agent.run"] as const},
  "agent.memories":{name:"agent.memories",description:"读取显式偏好和确定性聚合记忆；模型推断不会出现在此列表。",inputSchema:agentMemoriesInputSchema,commandSchema:agentMemoriesInputSchema,resultSchema:resourceDetailResultSchema,possibleEffects:["agent.run"] as const,idempotency:"not-applicable" as const,statusQuery:"operations.get",resolveEffects:()=>["agent.run"] as const},
  "notifications.list":{name:"notifications.list",description:"读取脱敏通知及其静默、关闭、延期或就绪状态。",inputSchema:notificationsInputSchema,commandSchema:notificationsInputSchema,resultSchema:resourceDetailResultSchema,possibleEffects:["notifications.read"] as const,idempotency:"not-applicable" as const,statusQuery:"operations.get",resolveEffects:()=>["notifications.read"] as const},
  "life.owned_items":{name:"life.owned_items",description:"读取本人显式加入的物品、售后截止日期、资料引用和维护事件；不会把所有购买记录当作库存。",inputSchema:ownedItemsInputSchema,commandSchema:ownedItemsInputSchema,resultSchema:resourceDetailResultSchema,possibleEffects:["life.item.read"] as const,idempotency:"not-applicable" as const,statusQuery:"operations.get",resolveEffects:()=>["life.item.read"] as const},
  "life.reviews":{name:"life.reviews",description:"读取程序生成、可按相同期间重算的跨领域复盘快照、覆盖率与证据引用。",inputSchema:lifeReviewsInputSchema,commandSchema:lifeReviewsInputSchema,resultSchema:resourceDetailResultSchema,possibleEffects:["life.review.read"] as const,idempotency:"not-applicable" as const,statusQuery:"operations.get",resolveEffects:()=>["life.review.read"] as const},
  "operations.get": {
