...
export const healthTrendResultSchema=z.object({metric_key:z.string(),points:z.array(z.object({id:stableId,occurred_on:localDate,occurred_at:instant.nullable(),value:canonicalDecimal,unit:z.string(),source_kind:z.string(),revision:z.number().int().positive()}).strict()),coverage:z.object({from:localDate.nullable(),to:localDate.nullable(),points:z.number().int().nonnegative(),truncated:z.boolean()}).strict(),as_of:instant}).strict();
