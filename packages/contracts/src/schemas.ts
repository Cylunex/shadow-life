...
const healthRecordRawSchema=z.object({id:stableId,record_type:z.enum(["body","wellbeing","sleep","workout","daily_activity","steps_interval","habit","lab","fitness_test"]),client_record_id:z.string(),current_version:z.number().int().nonnegative(),state:z.enum(["pending","normalized","failed","deleted","quarantined"]),parse_version:z.string().nullable(),source_instance_id:stableId,source_type:z.string().nullable(),source_instance_key:z.string().nullable()}).strict();
