...
export const purchaseSceneSchema=z.enum(["online_purchase","offline_purchase","delivery","dine_in","takeaway","home_cooked","drink","service","subscription","transport","entertainment","travel","other"]);
export const purchaseConsumableSchema=z.object({item_position:z.number().int().nonnegative().max(99),initial_quantity:positiveDecimal.optional(),quantity_unit:z.enum(["g","ml","count"]).optional(),quantity_label:z.string().trim().min(1).max(16).optional(),expected_daily_usage:positiveDecimal.optional()}).strict().refine(v=>(!v.initial_quantity&&!v.expected_daily_usage)||v.quantity_unit!==undefined,{message:"quantities need a unit"});
export const recordPurchaseInputSchema=datedFactSchema.extend({scene:purchaseSceneSchema,merchant_name_raw:z.string().trim().min(1).max(200).optional(),channel_name_raw:z.string().trim().min(1).max(100).optional(),place_ref:z.string().trim().min(1).max(300).optional(),rating:z.number().int().min(1).max(5).optional(),would_repeat:z.boolean().optional(),items:z.array(purchaseItemInputSchema).max(100).default([]),consumables:z.array(purchaseConsumableSchema).max(100).optional(),payment:purchasePaymentSchema.optional()}).strict().refine(factDateMatches,{path:["occurred_at"],message:"occurred_at does not fall on occurred_on in time_zone"}).refine(value=>value.items.length>0||value.merchant_name_raw!==undefined||value.note!==undefined,{message:"consumption needs an item, merchant, or note"}).refine(value=>!value.consumables||value.consumables.every(c=>c.item_position<value.items.length)&&new Set(value.consumables.map(c=>c.item_position)).size===value.consumables.length,{message:"consumable positions must identify distinct purchase items"});
...
export const correctPurchaseInputSchema=z.object({record_id:stableId,expected_revision:z.number().int().positive(),occurred_on:localDate,occurred_at:instant.optional(),time_zone:ianaTimeZone,scene:purchaseSceneSchema,merchant_name_raw:z.string().trim().min(1).max(200).optional(),channel_name_raw:z.string().trim().min(1).max(100).optional(),place_ref:z.string().trim().min(1).max(300).optional(),rating:z.number().int().min(1).max(5).optional(),would_repeat:z.boolean().optional(),note:z.string().trim().min(1).max(2_000).optional(),reason:z.string().trim().min(1).max(500)}).strict().refine(factDateMatches,{path:["occurred_at"],message:"occurred_at does not fall on occurred_on in time_zone"});
const purchaseItemChangeSchema=z.discriminatedUnion("action",[z.object({action:z.literal("append"),item:purchaseItemInputSchema}).strict(),z.object({action:z.literal("replace"),item_id:stableId,item:purchaseItemInputSchema}).strict()]);
export const updatePurchaseItemsInputSchema=z.object({record_id:stableId,expected_revision:z.number().int().positive(),detail_state:z.enum(["complete","folded","supplemented"]),folded_item_ids:z.array(stableId).max(100).default([]),changes:z.array(purchaseItemChangeSchema).min(1).max(100),reason:z.string().trim().min(1).max(500)}).strict().superRefine((value,context)=>{if(value.detail_state==="supplemented"&&value.folded_item_ids.length===0)context.addIssue({code:"custom",path:["folded_item_ids"],message:"supplemented details must identify the retained folded source rows"});});
...
export const saveRecipeInputSchema=z.object({recipe_id:stableId.optional(),expected_revision:z.number().int().positive().optional(),title:z.string().trim().min(1).max(200),servings:positiveDecimal,items:z.array(intakeItemInputSchema).min(1).max(100),instructions:z.string().trim().min(1).max(20_000).optional(),source_url:z.url().max(2000).refine(value=>/^https?:\/\//u.test(value),"source must be an HTTP(S) URL").nullable().optional(),state:z.enum(["active","archived"]).default("active"),reason:z.string().trim().min(1).max(500).optional()}).strict().superRefine((value,context)=>{if((value.recipe_id===undefined)!==(value.expected_revision===undefined))context.addIssue({code:"custom",message:"recipe_id and expected_revision must be supplied together"});if(value.recipe_id!==undefined&&value.reason===undefined)context.addIssue({code:"custom",path:["reason"],message:"updating a recipe needs a reason"});if(value.recipe_id===undefined&&value.reason!==undefined)context.addIssue({code:"custom",path:["reason"],message:"reason is only accepted when updating a recipe"});});
...
export const recordConsumableUseInputSchema=z.object({cycle_id:stableId,expected_revision:z.number().int().positive(),use_id:stableId.optional(),occurred_on:localDate,quantity:positiveDecimal,state:z.enum(["active","voided"]).default("active"),note:z.string().trim().min(1).max(2000).optional(),reason:z.string().trim().min(1).max(500).optional()}).strict().refine(v=>!v.use_id||!!v.reason,{message:"corrections need a reason"}).refine(v=>v.state!=="voided"||!!v.use_id,{message:"voiding needs use_id"});
export const consumableUseSchema=z.object({id:stableId,occurred_on:localDate,quantity:storedAmount,state:z.enum(["active","voided"]),note:z.string().nullable(),revision:z.number().int().positive()}).strict();
export const useCycleStatusSchema=z.object({usage_state:z.enum(["pending","in_use"]).optional(),quantity_label:z.string().nullable().optional(),note:z.string().nullable().optional(),estimated_remaining_quantity:storedAmount.nullable().optional(),uses:z.array(consumableUseSchema).optional(),uses_truncated:z.boolean().optional(),id:stableId,purchase_record_id:stableId.nullable(),purchase_item_id:stableId.nullable(),item_name:z.string(),started_on:localDate,ended_on:localDate.nullable(),state:z.enum(["active","completed","discarded","replenished"]),revision:z.number().int().positive(),initial_quantity:storedAmount.nullable(),quantity_unit:useCycleUnitSchema.nullable(),expected_daily_usage:storedAmount.nullable(),replenish_threshold:storedAmount.nullable(),replenish_lead_days:z.number().int().nonnegative().nullable(),time_zone:ianaTimeZone,match_mode:useCycleMatchModeSchema,match_value:z.string().nullable(),reminder_enabled:z.boolean(),consumed_quantity:storedAmount.nullable(),remaining_quantity:storedAmount.nullable(),projected_depletion_on:localDate.nullable(),balance_status:z.enum(["tracking_only","needs_specification","monitoring","replenish_now","depleted","completed","discarded","replenished"]),matched_intakes:z.number().int().nonnegative(),ignored_incompatible_intakes:z.number().int().nonnegative(),incompatible_units:z.array(z.string())}).strict();
export const moneyPlanningResultSchema=z.object({period:z.string().regex(/^\d{4}-(?:0[1-9]|1[0-2])$/u),window:z.object({start_on:localDate,end_on_exclusive:z.string().regex(/^\d{4,5}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01])$/u)}).strict(),budgets:z.array(z.object({id:stableId,category:z.string().nullable(),amount:storedAmount,currency:currencyCode,revision:z.number().int().positive(),spent_basis:z.literal("gross_expense"),spent:storedAmount}).strict()),recurring_plans:z.array(z.object({id:stableId,title:z.string(),amount:storedAmount.nullable(),currency:currencyCode,cadence:z.enum(["daily","weekly","monthly","yearly","interval"]),next_due_on:localDate,category:z.string().nullable(),active:z.boolean(),revision:z.number().int().positive(),time_zone:ianaTimeZone,interval_days:z.number().int().positive().nullable(),state:z.enum(["active","paused","ended"]),ended_on:localDate.nullable(),anchor_on:localDate,local_time:z.string().nullable(),missing_date_policy:z.enum(["skip","last_day"]),recurrence_rule:z.string()}).strict()),occurrences:z.array(z.object({id:stableId,plan_id:stableId,due_on:localDate,original_due_on:localDate,effective_due_on:localDate,state:z.enum(["pending","reminded","handled","dismissed","snoozed"]),linked_record_id:stableId.nullable(),feedback:z.record(z.string(),z.unknown()),snoozed_until:instant.nullable(),revision:z.number().int().positive()}).strict()),spending_intents:z.array(z.object({id:stableId,title:z.string(),expected_amount:storedAmount.nullable(),currency:currencyCode.nullable(),intended_on:localDate.nullable(),state:z.enum(["planned","purchased","cancelled"]),linked_record_id:stableId.nullable(),revision:z.number().int().positive()}).strict()),use_cycles:z.array(useCycleStatusSchema),limits:z.object({budgets:z.number().int().positive(),recurring_plans:z.number().int().positive(),occurrences:z.number().int().positive(),spending_intents:z.number().int().positive(),use_cycles:z.number().int().positive()}).strict(),truncated:z.object({budgets:z.boolean(),recurring_plans:z.boolean(),occurrences:z.boolean(),spending_intents:z.boolean(),use_cycles:z.boolean()}).strict(),as_of:instant}).strict();
...
export const healthObservationMetricSchema=z.enum(["weight","height","bmi","body_fat","fat_mass","lean_mass","muscle_mass","muscle_rate","skeletal_muscle","body_water","body_water_rate","bone_mass","bone_rate","visceral_fat","bmr","impedance_low","impedance_high","waist","chest","hip","heart_rate","blood_pressure_systolic","blood_pressure_diastolic","temperature","spo2","blood_glucose","lab_value","fitness_value"]);
...
export const setTripChecklistInputSchema=z.object({trip_id:stableId,expected_revision:z.number().int().positive().optional(),items:z.array(z.object({id:stableId.optional(),title:z.string().trim().min(1).max(200),state:z.enum(["needed","packed","skipped"]),note:z.string().trim().max(1_000).optional()}).strict()).max(200),reason:z.string().trim().min(1).max(500)}).strict();
...
const workspaceVisitSchema=z.object({id:stableId,trip_id:stableId.nullable(),place_name:z.string(),latitude:coordinateDecimal.nullable(),longitude:coordinateDecimal.nullable(),occurred_on:localDate,occurred_at:instant.nullable(),time_zone:ianaTimeZone,visibility:z.enum(["shared","private"]),revision:z.number().int().positive()}).strict().refine(value=>(value.latitude===null)===(value.longitude===null),{message:"workspace visit coordinates must be supplied together"});
...
export const travelWorkspaceResultSchema=z.object({
  ...
  visits:z.array(workspaceVisitSchema).max(1_000),
  ...
  checklist:z.object({id:stableId,trip_id:stableId,revision:z.number().int().positive(),items:z.array(z.object({id:stableId,title:z.string(),state:z.enum(["needed","packed","skipped"]),note:z.string().nullable()}).strict()).max(200)}).strict().nullable(),
...
export const reviseLibraryItemInputSchema=z.object({item_id:stableId,expected_revision:z.number().int().positive(),title:z.string().trim().min(1).max(300),text:z.string().min(1).max(100_000).optional(),url:z.url().max(2_000).optional(),tags:z.array(z.string().trim().min(1).max(64)).max(30).default([]),document_date:localDate.nullable().optional(),category:z.string().trim().min(1).max(80).nullable().optional(),source_processing_job_id:stableId.optional(),reason:z.string().trim().min(1).max(500)}).strict().refine(value=>value.text!==undefined||value.url!==undefined,{message:"revision needs text or url"});
...
export const queueLibraryProcessingInputSchema=z.object({item_id:stableId,source_asset_version_id:stableId,kind:z.enum(["text_extract","ocr","transcript","vision"]),requested_processor:z.string().trim().min(1).max(100)}).strict();
...
export const libraryVisionCandidateSchema=z.object({title:z.string().trim().min(1).max(300),document_date:localDate.nullable(),category:z.string().trim().min(1).max(80).nullable(),summary:z.string().trim().min(1).max(5_000),content:z.string().trim().min(1).max(30_000),locators:z.array(z.object({page:z.number().int().min(1).max(8),quote:z.string().trim().max(300).nullable()}).strict()).max(50)}).strict();
export const completeLibraryProcessingInputSchema=z.object({job_id:stableId,attempt:z.number().int().positive(),derived_asset_version_id:stableId,processor_version:z.string().trim().min(1).max(100),snippets:z.array(z.object({text:z.string().trim().min(1).max(20_000),locator:z.json()}).strict()).min(1).max(500),suggestion:libraryVisionCandidateSchema.optional()}).strict();
export const libraryProcessingQueueInputSchema=z.object({kind:z.enum(["text_extract","ocr","transcript","vision"]).optional(),limit:z.number().int().min(1).max(100).default(20)}).strict();
export const libraryProcessingQueueResultSchema=z.object({items:z.array(z.object({id:stableId,item_id:stableId,source_asset_version_id:stableId,kind:z.enum(["text_extract","ocr","transcript","vision"]),requested_processor:z.string().nullable(),state:z.enum(["queued","running"]),attempts:z.number().int().nonnegative(),lease_expires_at:instant.nullable(),created_at:instant,title:z.string(),media_type:z.string(),sha256:z.string().regex(/^[0-9a-f]{64}$/u),byte_size:z.string().regex(/^\d+$/u)}).strict()).max(100),as_of:instant}).strict();
export const libraryItemResultSchema=z.object({
  ...
  revisions:z.array(z.object({item_id:stableId,revision:z.number().int().positive(),text:z.string().nullable(),url:z.string().nullable(),tags:z.array(z.string()),document_date:localDate.nullable(),category:z.string().nullable(),source_processing_job_id:stableId.nullable(),reason:z.string().nullable(),created_at:instant}).strict()),
  ...
  processing_jobs:z.array(z.object({id:stableId,item_id:stableId,source_asset_version_id:stableId,kind:z.enum(["text_extract","ocr","transcript","vision"]),requested_processor:z.string(),state:z.enum(["queued","running","completed","failed"]),attempts:z.number().int().nonnegative(),derived_asset_version_id:stableId.nullable(),processor_version:z.string().nullable(),suggestion:libraryVisionCandidateSchema.nullable(),last_error:z.string().nullable(),started_at:instant.nullable(),finished_at:instant.nullable(),lease_expires_at:instant.nullable(),created_at:instant,updated_at:instant}).strict()),
...
export const saveActionItemInputSchema=z.object({action_item_id:stableId.optional(),expected_revision:z.number().int().positive().optional(),project_id:stableId,title:z.string().trim().min(1).max(200),due_on:localDate.nullable().optional(),scheduled_at:instant.nullable().optional(),scheduled_time_zone:ianaTimeZone.nullable().optional(),state:z.enum(["open","completed","cancelled"]).default("open"),recurring_occurrence_id:stableId.optional(),health_habit_id:stableId.optional()}).strict().superRefine((value,context)=>{if((value.action_item_id===undefined)!==(value.expected_revision===undefined))context.addIssue({code:"custom",message:"action_item_id and expected_revision must be supplied together"});if(value.recurring_occurrence_id&&value.health_habit_id)context.addIssue({code:"custom",message:"an action item can mirror at most one existing due source"});if(Boolean(value.scheduled_at)!==Boolean(value.scheduled_time_zone))context.addIssue({code:"custom",path:["scheduled_at"],message:"scheduled_at and scheduled_time_zone must be supplied together"});if(value.scheduled_at&&value.scheduled_time_zone&&(!value.due_on||!factDateMatches({occurred_on:value.due_on,occurred_at:value.scheduled_at,time_zone:value.scheduled_time_zone})))context.addIssue({code:"custom",path:["scheduled_at"],message:"scheduled time must fall on due_on in its time zone"});});
...
export const setFoodStockInputSchema=z.object({lot_id:stableId.optional(),expected_revision:z.number().int().positive().optional(),name:z.string().trim().min(1).max(200),quantity:z.string().regex(/^(?:0|[1-9]\d*)(?:\.\d{1,6})?$/u),unit:z.string().trim().min(1).max(32),expires_on:localDate.nullable().optional(),note:z.string().trim().max(500).nullable().optional()}).strict().refine(value=>(value.lot_id===undefined)===(value.expected_revision===undefined),{message:"lot_id and expected_revision must be supplied together"});
...
export const writeCommandSchemas = {
  ...
  "life.update_purchase_items":updatePurchaseItemsInputSchema,
  ...
  "money.record_consumable_use":recordConsumableUseInputSchema,
  ...
  "travel.set_checklist":setTripChecklistInputSchema,
  ...
  ,"life.set_food_stock":setFoodStockInputSchema
...
export const resourceReferenceSchema = z.object({
  type: z.enum(["meal", "meal_template", "food", "recipe", "personal_alias", "intake_item", "consumption_record", "purchase", "purchase_item", "money_entry", "refund", "budget", "recurring_plan", "recurring_occurrence", "spending_intent", "use_cycle", "service_card", "service_card_use", "consumable_use", "money_import_batch", "money_import_candidate", "money_import_rule", "health_measurement", "health_workout_session", "health_raw", "health_sync_cursor", "health_plan", "trip", "trip_segment", "reservation", "visit", "place", "travel_map", "trip_track", "trip_day_plan", "trip_checklist", "trip_member", "trip_plan_version", "trip_run", "trip_stop_outcome", "library_item", "library_annotation", "library_derivation", "library_processing_job", "library_reading_state", "library_legacy_link", "agent_context_pack", "agent_memory", "notification_preferences", "notification", "notification_installation", "notification_delivery", "owned_item", "owned_item_event", "life_review", "life_project", "action_item", "meal_plan", "shopping_list", "shopping_list_item", "food_stock_lot", "money_fx_snapshot", "shared_expense_allocation", "source", "thread", "run", "task"]),
  id: stableId,
...
export const lifeTodayResultSchema=z.object({date:localDate,domains:z.object({
  meals:z.object({count:z.number().int().nonnegative(),freshness:instant.nullable(),nutrition:z.object({energy_kcal:canonicalDecimal.nullable(),protein_g:canonicalDecimal.nullable(),carb_g:canonicalDecimal.nullable(),fat_g:canonicalDecimal.nullable(),total_items:z.number().int().nonnegative(),known_energy_items:z.number().int().nonnegative(),complete_macros:z.boolean()}).strict().optional()}).strict().optional(),
...
export const lifeTimelineItemSchema=z.object({domain:z.enum(["meals","money","health","travel","library","items","plans"]),kind:z.string(),id:stableId,happened_at:instant,happened_on:localDate.optional(),title:z.string(),amount:storedAmount.optional(),currency:currencyCode.optional(),record_id:stableId.optional()}).strict();
...
export const lifeDayInputSchema=z.object({date:localDate,time_zone:ianaTimeZone,limit:z.number().int().min(1).max(100).default(30),cursor:z.string().max(1000).optional()}).strict();
export const lifeDayRelatedSchema=z.object({domain:z.enum(["meals","money","travel","library","items"]),kind:z.string(),id:stableId,title:z.string()}).strict();
export const lifeDayResultSchema=z.object({date:localDate,time_zone:ianaTimeZone,authorized_domains:z.array(lifeTimelineItemSchema.shape.domain).max(7),items:z.array(lifeTimelineItemSchema.extend({related:z.array(lifeDayRelatedSchema).max(20)})).max(100),total:z.number().int().nonnegative(),next_cursor:z.string().nullable(),as_of:instant}).strict();
export const lifeMemoriesInputSchema=z.object({date:localDate,time_zone:ianaTimeZone}).strict();
export const lifeMemoriesResultSchema=z.object({date:localDate,time_zone:ianaTimeZone,authorized_domains:z.array(lifeTimelineItemSchema.shape.domain).max(7),items:z.array(lifeTimelineItemSchema.extend({years_ago:z.number().int().min(1).max(5)})).max(100),as_of:instant}).strict();
...
export const moneyImportMonthInputSchema=z.object({period:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/u)}).strict();
export const moneyImportMonthResultSchema=z.object({period:z.string(),batches:z.array(z.object({id:stableId,source_name:z.string(),status:z.enum(["review","completed"]),created_at:instant,total:z.number().int().nonnegative(),unconfirmed:z.number().int().nonnegative(),duplicates:z.number().int().nonnegative(),uncategorized:z.number().int().nonnegative(),refunds_unlinked:z.number().int().nonnegative()}).strict()).max(100),totals:z.object({unconfirmed:z.number().int().nonnegative(),duplicates:z.number().int().nonnegative(),uncategorized:z.number().int().nonnegative(),refunds_unlinked:z.number().int().nonnegative()}).strict(),as_of:instant}).strict();
...
export const workoutProgressionInputSchema=z.object({plan_id:stableId.optional()}).strict();
export const workoutProgressionResultSchema=z.object({plans:z.array(z.object({id:stableId,title:z.string(),revision:z.number().int().positive(),status:z.enum(["no_execution","missing_data","hold","increase_duration"]),reason:z.string(),next_duration_minutes:z.number().int().nonnegative().nullable(),evidence:z.array(z.object({id:stableId,occurred_on:localDate,session_type:z.string(),duration_minutes:z.number().int().nonnegative().nullable(),rpe:z.number().int().min(0).max(10).nullable(),revision:z.number().int().positive()}).strict())}).strict()),as_of:instant}).strict();
export const healthSleepInsightsInputSchema=z.object({to:localDate,days:z.union([z.literal(7),z.literal(30),z.literal(90)]).default(30)}).strict();
export const healthSleepInsightsResultSchema=z.object({from:localDate,to:localDate,nights:z.number().int().nonnegative(),bedtime_nights:z.number().int().nonnegative(),average_minutes:z.number().int().nonnegative().nullable(),at_least_seven_hours:z.number().int().nonnegative(),average_efficiency_percent:z.number().int().min(0).max(100).nullable(),efficiency_nights:z.number().int().nonnegative(),deep_percent:z.number().int().nonnegative().nullable(),rem_percent:z.number().int().nonnegative().nullable(),average_bedtime:z.string().regex(/^\d{2}:\d{2}$/u).nullable(),bedtime_variation_minutes:z.number().int().nonnegative().nullable(),source_types:z.array(z.string()),as_of:instant}).strict();
const healthRecordRawSchema=z.object({id:stableId,record_type:healthRawRecordTypeSchema,client_record_id:z.string(),current_version:z.number().int().nonnegative(),state:z.enum(["pending","normalized","failed","deleted","quarantined"]),parse_version:z.string().nullable(),source_instance_id:stableId,source_type:z.string().nullable(),source_instance_key:z.string().nullable()}).strict();
...
export const healthDailyResultSchema=z.object({occurred_on:localDate,algorithm_version:z.string(),result:z.object({
  ...
  workouts:z.array(z.object({id:stableId,session_type:z.string(),time_zone:ianaTimeZone.optional(),started_at:instant.nullable(),duration_minutes:z.number().int().nonnegative().nullable(),distance_km:canonicalDecimal.nullable(),calories_kcal:canonicalDecimal.nullable(),rpe:z.number().int().min(0).max(10).nullable(),heart_rate_avg:z.number().int().positive().nullable(),detail:z.json().nullable()}).strict()).max(1_000),
...
export const healthReleaseHistoryInputSchema=z.object({from:localDate,to:localDate,limit:z.number().int().min(1).max(1000).default(1000)}).strict().refine(value=>value.to>=value.from&&Date.parse(value.to)-Date.parse(value.from)<=365*86400000,{path:["to"],message:"choose an ordered window of at most 366 days"});
export const healthReleaseHistoryResultSchema=z.object({from:localDate,to:localDate,items:z.array(z.object({id:stableId,occurred_on:localDate,time_zone:ianaTimeZone,started_at:instant.nullable(),duration_minutes:z.number().nonnegative().nullable(),done_count:z.number().int().nonnegative(),explicit_denial:z.boolean(),note:z.string().nullable(),source_type:z.string(),revision:z.number().int().positive()}).strict()),total_count:z.number().int().nonnegative(),recorded_days:z.number().int().nonnegative(),latest_on:localDate.nullable(),truncated:z.boolean(),as_of:instant}).strict();
...
export const healthTrendResultSchema=z.object({metric_key:z.string(),points:z.array(z.object({id:stableId,occurred_on:localDate,occurred_at:instant.nullable(),value:canonicalDecimal,unit:z.string(),source_kind:z.string(),original_field:z.string().nullable().optional(),revision:z.number().int().positive()}).strict()),coverage:z.object({from:localDate.nullable(),to:localDate.nullable(),points:z.number().int().nonnegative(),truncated:z.boolean()}).strict(),as_of:instant}).strict();
