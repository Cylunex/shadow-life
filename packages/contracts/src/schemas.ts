...
export const healthSourcesResultSchema=z.object({items:z.array(z.object({id:stableId,source_type:z.string(),instance_key:z.string(),permission_state:z.string(),sync_epoch:z.number().int().positive(),fingerprint:z.string().nullable(),cursors:z.array(z.object({device_id:z.string(),record_type:z.string(),state:z.string(),sync_epoch:z.number().int().positive(),updated_at:instant}).strict())}).strict()),as_of:instant}).strict();
