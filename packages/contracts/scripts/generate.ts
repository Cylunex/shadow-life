...
import { domainRecordsResultSchema, healthSourcesResultSchema, lifeSearchResultSchema, lifeTimelineResultSchema, lifeTodayResultSchema, notificationsResultSchema, planningAgendaResultSchema } from "../src/schemas.js";
...
function defineObject(schema: JsonSchema, name: string): void {
  ...
  const fields = Object.entries(properties).map(([wire, value]) => {
    ...
    const optional = !required.has(wire) && !type.endsWith("?");
    return `  @SerialName(${JSON.stringify(wire)}) val ${local}: ${type}${optional ? "? = null" : ""}`;
  });
  kotlinDefinitions.push(`@Serializable\ndata class ${name}(\n${fields.join(",\n")}\n)`);
}
