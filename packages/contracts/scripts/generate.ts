...
import { domainRecordsResultSchema, planningAgendaResultSchema } from "../src/schemas.js";
...
function defineObject(schema: JsonSchema, name: string): void {
  ...
  const fields = Object.entries(properties).map(([wire, value]) => {
    ...
    const child = value.type === "array" ? `${name}${pascal(singular(wire))}` : `${name}${pascal(wire)}`;
