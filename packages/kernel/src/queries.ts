...
export class QueryService {
  ...
  async moneyPlanning(context:RequestContext,period:string){if(!context.effects.has("money.entry.read"))throw permissionDenied("money.entry.read");if(!/^\d{4}-\d{2}$/u.test(period))throw invalidInput("period must be YYYY-MM",["period"]);return this.unitOfWork.read(store=>store.moneyPlanning(context.subjectId,period));}
