...
class MemoryUnitOfWork implements UnitOfWork {
  ...
  async transaction<T>(work: (store: TransactionStore) => Promise<T>): Promise<T> {
    ...
    const store: TransactionStore = {
      ...
      lifeRecord:async()=>undefined,moneyPlanning:async()=>({}),moneyImportReview:async()=>undefined,healthDaily:async()=>undefined,healthRecord:async()=>undefined,travelTrip:async()=>undefined,libraryItem:async()=>undefined,agentPersonalContext:async()=>({aliases:[],mealTemplates:[]})
    };
