...
class MemoryUnitOfWork implements UnitOfWork {
  ...
  async transaction<T>(work: (store: TransactionStore) => Promise<T>): Promise<T> {
    ...
    const store: TransactionStore = {
      ...
      lifeRecord:async()=>undefined,moneyPlanning:async()=>({}),moneyImportReview:async()=>undefined,healthDaily:async()=>undefined,healthRecord:async()=>undefined,travelTrip:async()=>undefined,travelWorkspace:async()=>({}),travelExportData:async()=>undefined,libraryItem:async()=>undefined,libraryProcessingQueue:async()=>({items:[]}),agentContextPack:async()=>undefined,agentMemories:async()=>[],notifications:async()=>({items:[]}),ownedItems:async()=>({items:[]}),lifeReviews:async()=>({items:[]}),lifeProjects:async()=>({items:[]}),mealPlanning:async()=>({meal_plans:[],shopping_lists:[]}),purchaseItems:async()=>({items:[]}),foreignEntries:async()=>({items:[]}),agentPersonalContext:async()=>({aliases:[],mealTemplates:[]})
    };
